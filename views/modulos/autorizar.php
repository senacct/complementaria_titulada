...
if(isset($_SESSION['prc_ciuser'])){
    ...
    if($perfil == '1'){
    $sqlt = "SELECT fc.id id, fc.lugar, fc.ofertaabierta, fc.direccion, fc.naprendices, fr.codigo, fr.nombre, fr.horas, us.nombre instructor, fc.idempresa, cd.ciudad, fc.finicia, fc.ffinaliza FROM fcaracterizacion fc INNER JOIN formaciones fr ON fc.idprograma = fr.id INNER JOIN usuarios us ON fc.idusuario = us.id INNER JOIN ciudades cd ON fc.ciudad = cd.id  WHERE  fc.ccentro = '$centro' AND fc.historico = '0' AND fc.estado = '3' ORDER BY fc.id ASC";
    }else{
    $sqlt = "SELECT fc.id id, fc.lugar, fc.ofertaabierta, fc.direccion, fc.naprendices, fr.codigo, fr.nombre, fr.horas, us.nombre instructor, fc.idempresa , cd.ciudad, fc.finicia, fc.ffinaliza FROM fcaracterizacion fc INNER JOIN formaciones fr ON fc.idprograma = fr.id INNER JOIN usuarios us ON fc.idusuario = us.id INNER JOIN ciudades cd ON fc.ciudad = cd.id  WHERE fc.coordinacion = '$coordinacion' AND fc.historico = '0' AND fc.ccentro = '$centro' AND fc.estado = '3' ORDER BY fc.id ASC";
    }
	$stmt = Conexion::conectar()->prepare($sqlt); 
	$stmt -> execute();
	if($stmt -> rowcount() > 0){
