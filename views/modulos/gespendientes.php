...
function seleccionar(){
    if(!isset($_SESSION['prc_ciuser'])){
         ...
         $sqlt = "SELECT fc.id, rr.tpendiente, rr.sgs, rr.tsgs FROM fcaracterizacion fc INNER JOIN reservaregistro rr ON fc.id = rr.idregistro WHERE rr.idusuario = '$idusuario' AND fc.estado = '5' AND pendiente = '1';";
         ...
         if($stmt->rowCount() > 0){
            ...
            foreach ($registros as $key => $value){
                 ...
                 $sgs = $value['sgs'];
                 $tsgs = $value['tsgs'];
                 verprogramar($id, $tpendiente, $tban, $sgs, $tsgs);
                 $tban = '1';
            }
         } 
     }  
...
if(!isset($_SESSION['prc_ciuser'])){
     ...
     if($stmt->rowCount() > 0){ 
        ...
        $sqlt= "INSERT INTO reservaregistro (idusuario, idregistro, estado ) VALUES ('$idusuario','$idregistro', '1');";
        $stmt = Conexion::conectar()->prepare($sqlt);
        $stmt -> execute();
        $respuesta = 0;
     }
...
function verprogramar($id, $tpendiente, $tban, $sgs, $tsgs){ 
if(isset($_SESSION['prc_ciuser'])){
    ...
    foreach ($registros as $key => $value){
        ...
        <div class="col  col-md-auto col-sm-12">'.$datos.'</div>
            ...
            if($sgs == '1'){
                $cabecera .='<div class="card border-light mb-3">
                              <div class="card-body">'.$tpendiente.'
                                 </div>
                            </div>';
             }else{
                $cabecera .='<div class="card text-white bg-dark">
                              <div class="card-body">'.$tpendiente.'
                                 </div>
                            </div>';
            }       
            if($sgs == '1'){      
                $cabecera .='<div class="card border-light mb-3">
                               <div class="card-body"><span style="color:blue" class="font-weight-bold">RESPUESTA: </span>'.$tsgs.'
                               </div>
                            </div>';
            }  
...
$dias=array(0=>"Domingo", "Lunes", "Martes", "Miercoles", "Jueves", "Viernes", "Sabado" );
  $sqlt = "SELECT ano, mes, diasemana, horas, inicia, finaliza, dia, festivo, estado, novedad FROM programacion WHERE idficha = '$idficha' AND estado = '1' ORDER BY mes, inicia, finaliza, diasemana, dia ASC";
        ...
        $lista .='</table>';   
        $lista .='</div>';
        $lista .= '<div class="btn-group" role="group" aria-label="Basic example"><button type="button" id="btnprog'.$idficha.'" class="btn btn-success" onClick="programadoficha('.$idficha.',\'0\');">Programado</button>';
        $lista .= '<button type="button" class="btn btn-warning" onClick="prenovedad('.$idficha.',\'0\');">Reportar Novedad al Instructor</button></div>';  
        $lista .='</div> 
        </div> 
          </div>';      
    } 
