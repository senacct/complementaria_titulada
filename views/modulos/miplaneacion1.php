...
//include_once "models/conexion.php"; 
 echo '<script src="'.SERVERURL.'views/js/miplaneacion.02.js"></script>
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Mi Palneación</li>
      </ol>      
';
...
if(isset($_SESSION['prc_ciuser'])){
    ...
    $sqlt = "SELECT fc.numero, fc.alarmada, cr.nombre coordinacion, fc.codempresa, fc.controlada, fc.id id, fc.lugar, fc.direccion, fc.naprendices, fr.codigo, fr.nombre, fr.horas, us.nombre instructor, be.nestado, be.estado estado, be.fecha, em.nombre empresa, cd.ciudad, fc.finicia, fc.ffinaliza FROM fcaracterizacion fc INNER JOIN formaciones fr ON fc.idprograma = fr.id INNER JOIN usuarios us ON fc.idusuario = us.id INNER JOIN empresas em ON fc.idempresa = em.id  INNER JOIN ciudades cd ON fc.ciudad = cd.id INNER JOIN bitacoraestados be ON fc.id = be.idficha AND fc.estado = be.estado INNER JOIN coordinaciones cr ON fc.coordinacion = cr.id WHERE fc.ccentro = '$centro' AND fc.usuario = '$usuario' AND fc.historico = '0' AND fc.estado = be.estado AND be.visible = '1'  ORDER BY fc.id ASC";
            ...
            <?php if(!empty($registros)) { ?>
                <?php foreach ($registros as $key => $value) { 
                    ...
                    if($value['alarmada'] == 1){
                        ...
                        <td><?php echo $value['codempresa']; ?></td> 
